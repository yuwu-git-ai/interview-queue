...
- v1 大屏与语音**均显示/播报全名**（叫号广播必须能被同学认领，"张\* 请前往…"无法识别）。姓名脱敏切换留作后置增强。
...
**触发规则（硬性）**：声音**只在面试官点击后**发出。
1. 唯一发声入口 = 面试官控制台「呼叫下一位」「再次呼叫」→ 后端生成 `type: call|recall` 公告 → 等候室大屏轮询发现**新公告 id** → 播报。声音只出现在等候室大屏这台机器。
2. **绝不自动响**：新候选人扫码登记不响、页面刷新不响、轮询到普通状态变化不响、后端启动/重置不响。
3. 播报文本含教室指引（"请前往 事业部(教210)"）。

其余技术要点：
- 中文发音人优选：`zh-CN` → `zh`，按 voices 列表选含 "Chinese/中文/普通话" 项，取 lang 最长匹配。
- 防重复：同一公告 id 只播一次（大屏记忆 lastSpokenId）；重播仅由再次手动触发（面试官再次呼叫 或 大屏重播钮）。
- 队列调度：连续公告按序排入队列，speak 完成再播下一条；页面切走不累积爆队列；全局静音开关。
